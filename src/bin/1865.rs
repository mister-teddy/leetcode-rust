...
impl FindSumPairs {
    fn new(nums1: Vec<i32>, nums2: Vec<i32>) -> Self {
        ...
        for num2 in nums2.iter() {
            *nums2_map.entry(*num2).or_insert(0) += 1;
        }
        Self {
            nums2_map,
            nums1,
    ...
    fn add(&mut self, index: i32, val: i32) {
        let num2 = self.nums2[index as usize];
        self.nums2_map.entry(num2).and_modify(|it| *it -= 1);
        *self.nums2_map.entry(num2 + val).or_insert(0) += 1;
    ...
    fn count(&self, tot: i32) -> i32 {
        ...
        for num1 in self.nums1.iter() {
            let num2 = tot - num1;
            if num2 > 0 {
                res += self.nums2_map.get(&num2).unwrap_or(&0);
            }
        }
        res
    }
