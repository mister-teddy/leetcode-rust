...
impl Solution {
    pub fn min_time_to_reach(move_time: Vec<Vec<i32>>) -> i32 {
        ...
        loop {
            if let Some((time, (i, j, twice))) = queue.pop() {
                if -time < res[i][j] {
                    res[i][j] = -time;
                    let seconds = if twice { 1 } else { 2 };
                    if i > 0 {
                        queue.push((time.min(-move_time[i - 1][j]) - seconds, (i - 1, j, !twice)));
                    }
                    if j > 0 {
                        queue.push((time.min(-move_time[i][j - 1]) - seconds, (i, j - 1, !twice)));
                    }
                    if i < n - 1 {
                        queue.push((time.min(-move_time[i + 1][j]) - seconds, (i + 1, j, !twice)));
                    }
